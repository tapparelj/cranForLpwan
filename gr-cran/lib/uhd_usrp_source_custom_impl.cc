/* -*- c++ -*- */
/*
 * Copyright 2023 gr-custom_tools author.
 *
 * SPDX-License-Identifier: GPL-3.0-or-later
 */
#include "gr_uhd_common.h"
#include "uhd_usrp_source_custom_impl.h"
...
namespace gr {
namespace cran {
uhd_usrp_source_custom::sptr uhd_usrp_source_custom::make(const ::uhd::device_addr_t& device_addr,
                                    const ::uhd::stream_args_t& stream_args,
                                    const bool issue_stream_cmd_on_start)
{
    gr::uhd::check_abi();
    return uhd_usrp_source_custom::sptr(new uhd_usrp_source_custom_impl(
        device_addr, stream_args_ensure(stream_args), issue_stream_cmd_on_start));
}

uhd_usrp_source_custom_impl::uhd_usrp_source_custom_impl(const ::uhd::device_addr_t& device_addr,
                                   const ::uhd::stream_args_t& stream_args,
                                   const bool issue_stream_cmd_on_start)
          ...
          gr::prefs::singleton()->get_long("uhd", "logging_interval_ms", 750))
{
    ...
    register_msg_cmd_handler(gr::uhd::cmd_tag_key(),
                             [this](const pmt::pmt_t& tag, const int, const pmt::pmt_t&) {
                                 this->_cmd_handler_tag(tag);
                             });
}

uhd_usrp_source_custom_impl::~uhd_usrp_source_custom_impl() {}

::uhd::dict<std::string, std::string> uhd_usrp_source_custom_impl::get_usrp_info(size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_usrp_rx_info(chan);
}

void uhd_usrp_source_custom_impl::set_subdev_spec(const std::string& spec, size_t mboard)
{
    return _dev->set_rx_subdev_spec(spec, mboard);
}

std::string uhd_usrp_source_custom_impl::get_subdev_spec(size_t mboard)
{
    return _dev->get_rx_subdev_spec(mboard).to_string();
}

void uhd_usrp_source_custom_impl::set_samp_rate(double rate)
{
...
double uhd_usrp_source_custom_impl::get_samp_rate(void)
{
    return _dev->get_rx_rate(_stream_args.channels[0]);
}

::uhd::meta_range_t uhd_usrp_source_custom_impl::get_samp_rates(void)
{
    return _dev->get_rx_rates(_stream_args.channels[0]);
}

::uhd::tune_result_t
uhd_usrp_source_custom_impl::set_center_freq(const ::uhd::tune_request_t tune_request, size_t chan)
{
    chan = _stream_args.channels[chan];
...
::uhd::tune_result_t
uhd_usrp_source_custom_impl::_set_center_freq_from_internals(size_t chan, pmt::pmt_t direction)
{
    if (pmt::eqv(direction, gr::uhd::direction_tx())) {
...
double uhd_usrp_source_custom_impl::get_center_freq(size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_rx_freq(chan);
}

::uhd::freq_range_t uhd_usrp_source_custom_impl::get_freq_range(size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_rx_freq_range(chan);
}

void uhd_usrp_source_custom_impl::set_gain(double gain, size_t chan, pmt::pmt_t direction)
{
    chan = _stream_args.channels[chan];
    if (pmt::eqv(direction, gr::uhd::direction_tx())) {
...
void uhd_usrp_source_custom_impl::set_gain(double gain, const std::string& name, size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->set_rx_gain(gain, name, chan);
}

void uhd_usrp_source_custom_impl::set_rx_agc(const bool enable, size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->set_rx_agc(enable, chan);
}

void uhd_usrp_source_custom_impl::set_normalized_gain(double norm_gain, size_t chan)
{
    _dev->set_normalized_rx_gain(norm_gain, chan);
}

double uhd_usrp_source_custom_impl::get_gain(size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_rx_gain(chan);
}

double uhd_usrp_source_custom_impl::get_gain(const std::string& name, size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_rx_gain(name, chan);
}

double uhd_usrp_source_custom_impl::get_normalized_gain(size_t chan)
{
    return _dev->get_normalized_rx_gain(chan);
}

std::vector<std::string> uhd_usrp_source_custom_impl::get_gain_names(size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_rx_gain_names(chan);
}

::uhd::gain_range_t uhd_usrp_source_custom_impl::get_gain_range(size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_rx_gain_range(chan);
}

::uhd::gain_range_t uhd_usrp_source_custom_impl::get_gain_range(const std::string& name, size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_rx_gain_range(name, chan);
}

bool uhd_usrp_source_custom_impl::has_power_reference(size_t chan)
{
...
void uhd_usrp_source_custom_impl::set_power_reference(double power_dbm, size_t chan)
{
#ifdef UHD_USRP_MULTI_USRP_POWER_LEVEL
    if (chan >= _stream_args.channels.size()) {
        throw std::out_of_range("Invalid channel: " + std::to_string(chan));
...
double uhd_usrp_source_custom_impl::get_power_reference(size_t chan)
{
#ifdef UHD_USRP_MULTI_USRP_POWER_LEVEL
    if (chan >= _stream_args.channels.size()) {
        throw std::out_of_range("Invalid channel: " + std::to_string(chan));
...
::uhd::meta_range_t uhd_usrp_source_custom_impl::get_power_range(size_t chan)
{
#ifdef UHD_USRP_MULTI_USRP_POWER_LEVEL
    if (chan >= _stream_args.channels.size()) {
        throw std::out_of_range("Invalid channel: " + std::to_string(chan));
...
void uhd_usrp_source_custom_impl::set_antenna(const std::string& ant, size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->set_rx_antenna(ant, chan);
}

std::string uhd_usrp_source_custom_impl::get_antenna(size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_rx_antenna(chan);
}

std::vector<std::string> uhd_usrp_source_custom_impl::get_antennas(size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_rx_antennas(chan);
}

void uhd_usrp_source_custom_impl::set_bandwidth(double bandwidth, size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->set_rx_bandwidth(bandwidth, chan);
}

double uhd_usrp_source_custom_impl::get_bandwidth(size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_rx_bandwidth(chan);
}

::uhd::freq_range_t uhd_usrp_source_custom_impl::get_bandwidth_range(size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_rx_bandwidth_range(chan);
}

std::vector<std::string> uhd_usrp_source_custom_impl::get_lo_names(size_t chan)
{
...
const std::string uhd_usrp_source_custom_impl::get_lo_source(const std::string& name, size_t chan)
{
#ifdef UHD_USRP_MULTI_USRP_LO_CONFIG_API
    chan = _stream_args.channels[chan];
...
std::vector<std::string> uhd_usrp_source_custom_impl::get_lo_sources(const std::string& name,
                                                          size_t chan)
{
#ifdef UHD_USRP_MULTI_USRP_LO_CONFIG_API
    chan = _stream_args.channels[chan];
...
void uhd_usrp_source_custom_impl::set_lo_source(const std::string& src,
...
bool uhd_usrp_source_custom_impl::get_lo_export_enabled(const std::string& name, size_t chan)
{
#ifdef UHD_USRP_MULTI_USRP_LO_CONFIG_API
    chan = _stream_args.channels[chan];
...
void uhd_usrp_source_custom_impl::set_lo_export_enabled(bool enabled,
                                             const std::string& name,
                                             size_t chan)
{
#ifdef UHD_USRP_MULTI_USRP_LO_CONFIG_API
...
::uhd::freq_range_t uhd_usrp_source_custom_impl::get_lo_freq_range(const std::string& name,
                                                        size_t chan)
{
#ifdef UHD_USRP_MULTI_USRP_LO_CONFIG_API
    chan = _stream_args.channels[chan];
...
double uhd_usrp_source_custom_impl::get_lo_freq(const std::string& name, size_t chan)
{
#ifdef UHD_USRP_MULTI_USRP_LO_CONFIG_API
    chan = _stream_args.channels[chan];
...
double uhd_usrp_source_custom_impl::set_lo_freq(double freq, const std::string& name, size_t chan)
{
#ifdef UHD_USRP_MULTI_USRP_LO_CONFIG_API
    chan = _stream_args.channels[chan];
...
void uhd_usrp_source_custom_impl::set_auto_dc_offset(const bool enable, size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->set_rx_dc_offset(enable, chan);
}

void uhd_usrp_source_custom_impl::set_dc_offset(const std::complex<double>& offset, size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->set_rx_dc_offset(offset, chan);
}

void uhd_usrp_source_custom_impl::set_auto_iq_balance(const bool enable, size_t chan)
{
    chan = _stream_args.channels[chan];
...
void uhd_usrp_source_custom_impl::set_iq_balance(const std::complex<double>& correction, size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->set_rx_iq_balance(correction, chan);
}

::uhd::sensor_value_t uhd_usrp_source_custom_impl::get_sensor(const std::string& name, size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_rx_sensor(name, chan);
}

std::vector<std::string> uhd_usrp_source_custom_impl::get_sensor_names(size_t chan)
{
    chan = _stream_args.channels[chan];
    return _dev->get_rx_sensor_names(chan);
}

::uhd::usrp::dboard_iface::sptr uhd_usrp_source_custom_impl::get_dboard_iface(size_t chan)
{
    chan = _stream_args.channels[chan];
...
std::vector<std::string> uhd_usrp_source_custom_impl::get_filter_names(const size_t chan)
{
    return _dev->get_rx_filter_names(chan);
}

::uhd::filter_info_base::sptr uhd_usrp_source_custom_impl::get_filter(const std::string& path,
                                                           const size_t chan)
{
    return _dev->get_rx_filter(path, chan);
}

void uhd_usrp_source_custom_impl::set_filter(const std::string& path,
...
void uhd_usrp_source_custom_impl::set_stream_args(const ::uhd::stream_args_t& stream_args)
{
...
void uhd_usrp_source_custom_impl::_cmd_handler_tag(const pmt::pmt_t& tag) { _tag_now = true; }

void uhd_usrp_source_custom_impl::set_start_time(const ::uhd::time_spec_t& time)
{
    _start_time = time;
...
void uhd_usrp_source_custom_impl::issue_stream_cmd(const ::uhd::stream_cmd_t& cmd)
{
    _rx_stream->issue_stream_cmd(cmd);
    _tag_now = true;
}

void uhd_usrp_source_custom_impl::set_recv_timeout(const double timeout, const bool one_packet)
{
    _recv_timeout = timeout;
    _recv_one_packet = one_packet;
}

bool uhd_usrp_source_custom_impl::start(void)
{
...
void uhd_usrp_source_custom_impl::flush(void)
{
...
bool uhd_usrp_source_custom_impl::stop(void)
{
    std::lock_guard<std::recursive_mutex> lock(d_mutex);
...
std::vector<std::complex<float>> uhd_usrp_source_custom_impl::finite_acquisition(const size_t nsamps)
{
    if (_nchan != 1)
...
uhd_usrp_source_custom_impl::finite_acquisition_v(const size_t nsamps)
{
...
int uhd_usrp_source_custom_impl::work(int noutput_items,
                           ...
                           gr_vector_void_star& output_items)
{
    ...
    auto now = std::chrono::steady_clock::now();
    if(std::chrono::duration_cast<std::chrono::milliseconds>(now-_last_tag).count() > 500){

        auto new_time = get_time_last_pps(0);
        if(new_time != _time_last_pps && new_time.get_full_secs()>1700000000){
            //std::cout<<"time_last_pps: "<< std::setprecision(10)<< new_time.get_full_secs()<<" "<<new_time.get_frac_secs()<< std::setprecision(6)<<std::endl;
            auto diff_time = _time_last_pps;
            diff_time -= new_time;
            //std::cout<<"diff with previous:" << diff_time.get_full_secs()<<" "<<diff_time.get_frac_secs()<<std::endl;
            const pmt::pmt_t val = pmt::make_tuple(pmt::from_uint64(new_time.get_full_secs()),
                                                   pmt::from_double(new_time.get_frac_secs()));
            this -> add_item_tag(0, nitems_written(0), pmt::mp("time_last_pps"), val);
            _time_last_pps = new_time;
        }
        _last_tag = now;
    }
    return num_samps;
...
void uhd_usrp_source_custom_impl::setup_rpc()
{
#ifdef GR_CTRLPORT
