...
#ifndef INCLUDED_CRAN_LORA_FRAME_SYNC_IMPL_H
#define INCLUDED_CRAN_LORA_FRAME_SYNC_IMPL_H

// #define LORA_SYNC_DEBUG
...
namespace gr
{
    namespace cran
    {
        class lora_frame_sync_impl : public lora_frame_sync
        {
        private:
            enum DecoderState
            {
            ...
            int m_req_item_forecast; ///< number of items to forecast for the next iteration
            int m_synchro_read_offset;
            //FFT setup
            kiss_fft_cfg m_cfg;
            kiss_fft_cpx *m_fft_in;
            kiss_fft_cpx *m_fft_out;
        ...
        public:
            lora_frame_sync_impl(uint32_t center_freq, uint32_t bandwidth, uint8_t sf,
                               bool impl_head, std::vector<uint16_t> sync_word,
                               uint8_t os_factor, uint16_t preamble_len);
            ~lora_frame_sync_impl();
...
#endif /* INCLUDED_CRAN_LORA_FRAME_SYNC_IMPL_H */
