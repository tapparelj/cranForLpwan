...
namespace gr
{
    namespace lora_sdr
    {
                                   ...
                                   gr_vector_void_star &output_items)
        {
            ...
            if (tags.size())
            {
                ...
                else
                {
                    if (tags.size() >= 2)
                    {
                        ...
                        if (nitems_to_process == 0 && first_error)
                        {
                            std::cout << RED << "[hamming_dec_impl.cc] two tags with the same offset, something went wrong before!" << RESET << std::endl;
                            first_error = false;
                        }
                    }
            ...
            // std::cout<<"cr_app: "<<(int)cr_app<<std::endl;
                ...
                if (m_soft_decoding)
                {
                    ...
                    { // for all possible codeword
                        ...
                        { // for all codeword bits
                            ...
                            if ((bit and codeword_LLR[j] > 0) or (!bit and codeword_LLR[j] < 0))
                            { // if correct bit 1-->1 or 0-->0
                                cw_proba[n] += abs(codeword_LLR[j]);
                                // std::cout<<abs(codeword_LLR[j])<<" ";
                            }
                            else
                    ...
                    std::cout << "correct cw " << unsigned(cw_LUT[idx_max]) << " with proba " << cw_proba[idx_max] << " idxm " << unsigned(idx_max) << std::endl;
                    ...
                    //std::cout << (int)data_nibble_soft << std::endl;
