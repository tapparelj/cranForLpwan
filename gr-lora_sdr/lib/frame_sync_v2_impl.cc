...
namespace gr
{
    namespace lora_sdr
    {
        ...
        frame_sync_v2::sptr
        frame_sync_v2::make(uint32_t center_freq, uint32_t bandwidth, uint8_t sf,
                            bool impl_head, std::vector<uint16_t> sync_word,
                            uint8_t os_factor, uint16_t preamble_len)
        {
            return gnuradio::make_block_sptr<frame_sync_v2_impl>(center_freq, bandwidth,
         ...
         */
        frame_sync_v2_impl::frame_sync_v2_impl(uint32_t center_freq, uint32_t bandwidth,
                                               uint8_t sf, bool impl_head,
                                               std::vector<uint16_t> sync_word,
                                               uint8_t os_factor, uint16_t preamble_len)
            : gr::block("frame_sync_v2",
         ...
         */
        frame_sync_v2_impl::~frame_sync_v2_impl() {}

        void frame_sync_v2_impl::forecast(int noutput_items,
                                          gr_vector_int &ninput_items_required)
        {
            ninput_items_required[0] = m_buffer_read_offset + m_n_up * m_os_factor*(1u<<MAX_SF);
            // ninput_items_required[0] = 1;
        }
        void frame_sync_v2_impl::set_sf(int sf)
        {
            if(sf != m_sf){
                 std::cout << RED<<"[Error][Frame_sync_v2_impl.cc] "<<m_identity <<" Change of SF "<<(int)m_sf<<" to "<<sf<< " with Tag not supported yet. Proper buffer realignment required.\n"<<RESET;
            }
            m_sf = sf;
            m_bins_per_symbol = (uint32_t)(1u << m_sf);
            m_samples_per_symbol = m_bins_per_symbol * m_os_factor;
            m_max_snr_diff = 6; //TODO improve this decision
            ...
            // sync words
            build_upchirp(&m_ref_preamble[m_n_up * m_bins_per_symbol], m_sync_words[0], m_sf);
            build_upchirp(&m_ref_preamble[(m_n_up + 1) * m_bins_per_symbol], m_sync_words[1], m_sf);
            // downchirps
        ...
        void frame_sync_v2_impl::frame_info_handler(pmt::pmt_t frame_info)
        {
        ...
        std::pair<int, float> frame_sync_v2_impl::dechirp_and_fft(const gr_complex *samples, std::vector<gr_complex> *ref_chirp, uint16_t length)
        {
            // dechirp
            ...
            kiss_fft_cpx *fft_in = new kiss_fft_cpx[length];
            kiss_fft_cpx *fft_out = new kiss_fft_cpx[length];
            ...
            // get symbol value
            int symb_val = mod(std::round(float(max_it - fft_mag_sq) * ref_chirp->size() / length), ref_chirp->size());

            return std::pair<int, float>{symb_val, std::sqrt(*max_it)};
        }

        int frame_sync_v2_impl::my_roundf(float number)
        {
            int ret_val = (int)(number > 0 ? int(number + 0.5) : std::ceil(number - 0.5));
            return ret_val;
        }

        float frame_sync_v2_impl::determine_snr(const gr_complex *samples, const gr_complex *ref_downchirp, float *out_noise_pow, float *out_signal_pow, int number_of_samples, int additional_symbol_bin)
        {
            double tot_en = 0;
            ...
            {
                sig_and_noise_en_sum += fft_mag_sq[mod(max_idx+i,number_of_samples)]/ number_of_samples;
            }
            ...
            // std::cout << "[frame_sync_v2_impl.cc] "<<m_identity<<" sig_pow "<<sig_pow<<", noise_pow "<<noise_pow<<", snr "<<10 * log10(sig_pow / noise_pow)<<std::endl;
        ...
        float frame_sync_v2_impl::estimate_CFO_frac(gr_complex *samples)
        {
            int k0[m_n_up];
            float cfo_frac;
        ...
        float frame_sync_v2_impl::estimate_STO_frac(gr_complex *samples)
        {
            int k0;
            ...
            Y_1 = fft_mag_sq[mod(k0 - 1, 2 * m_bins_per_symbol)];
            Y0 = fft_mag_sq[k0];
            Y1 = fft_mag_sq[mod(k0 + 1, 2 * m_bins_per_symbol)];
        ...
        int frame_sync_v2_impl::general_work(int noutput_items,
                                             ...
                                             gr_vector_void_star &output_items)
        {
            ...
            if (finished())
            {
                std::cout << RED << "work done frame_sync" << RESET << std::endl;
                return WORK_DONE;
            }
            ...
            get_tags_in_window(tags, 0, 0, ninput_items[0], pmt::string_to_symbol("work_info"));
            if (tags.size())
            {       
                ...
                if (tags[0].offset > nitems_read(0) + m_buffer_read_offset)
                {
                        ...
                        //std::cout << "[frame_sync_v2_impl.cc] "<<m_identity <<" new frame tag: nitems_read "<<nitems_read(0)<<", tag offset " << tags[0].offset <<",m_buffer_read_offset "<<m_buffer_read_offset<<", max_items_to_process "<<max_items_to_process<< "\n";
                        if(max_items_to_process/m_samples_per_symbol < m_n_up )//if not enough samples until tag to try detecting the previous frame, just drop them and focus on the next
                        {
                            ...
                            //std::cout << "[frame_sync_v2_impl.cc] "<<m_identity <<" dropped "<<items_to_consume<<" items\n";
                            goto return_work;
                ...
                else if(tags[0].offset < nitems_read(0) + m_buffer_read_offset)//tag in buffer margin (should be a previously found frame begin tag still in buffer)
                {
                    if(tags[0].offset != m_last_tag_offset){
                        std::cout <<RED<< "[Error][frame_sync_v2_impl.cc] "<<m_identity <<" tag in the buffer margin! nitems_read "<<nitems_read(0) <<", tag offset " << tags[0].offset<<"\n"<<RESET;
                    }
                    if (tags.size() >= 2)
                    {
                        std::cout << "[frame_sync_v2_impl.cc] "<<m_identity <<" Two tags in a single input buffer, this is suspiciously close \n";
                ...
                else//we start a new frame
                {
                    ...
                    set_sf(sf);
                    m_frame_tag = tags[0];
                    std::cout<<"[frame_sync_impl.cc] new_frame Tag offset: "<<tags[0].offset<<std::endl;
                    ...
                    std::cout<<"[frame_sync_impl.cc] Tag relative offset: "<<tags[0].offset -nitems_read(0) <<", time firt sample of new work: "<<m_current_time.get_full_secs()<<"."<<m_current_time.get_frac_secs()<<"\n";

                    m_received_frame_tag = true;
                    //std::cout << "[frame_sync_v2_impl.cc] "<<m_identity<<" "<<work_id <<" new frame tag with correct offset " << tags[0].offset<<", nitemsread: "<<nitems_read(0) << " sf: " << sf << " work_id " << work_id <<" max_items_to_process " <<max_items_to_process<< "\n";
            ...
            case DETECT:
            {
                ...
                for (int symb_id = 0; symb_id < max_items_to_process/ m_samples_per_symbol - m_n_up + 1; symb_id++)
                {
                    ...
                    int diff_with_prev = abs(mod(abs(max_bin_new.first - m_prev_bin_idx.first) + (m_n_up * m_bins_per_symbol / 2), m_n_up * m_bins_per_symbol) - m_n_up * m_bins_per_symbol / 2);
                        ...
                        max_bin_new.second > 0)                                                 // and ignore maximums with 0 amplitude which appears when simulation with no noise
                    {
                        ...
                        // std::cout<<"[frame_sync_v2_impl.cc] symb_start idx: "<<nitems_read(0)+m_buffer_read_offset + symb_id * m_samples_per_symbol<<", max_bin_new: "<<max_bin_new.first<<" "<<max_bin_new.second<<std::endl;
            ...
            case SYNC:
            {
                ...
                int synchro_read_offset = m_buffer_read_offset - m_n_repetitions*m_samples_per_symbol;
                ...
                std::cout<<"Resync offset: "<< (int)( symb_rep_offset) * m_samples_per_symbol + m_samples_per_symbol + m_os_factor * (1)<<", and new one: "<<(int)synchro_read_offset<<std::endl;
                m_n_repetitions = 1;
                for (int candidate_id = 0; candidate_id < m_sync_candidates.size(); candidate_id++)
                {
                    ...
                    {
                        preamb_upchirps_down[ii] = in[synchro_read_offset + candidate_id * m_samples_per_symbol  + m_os_factor * ii]; //+m_samples_per_symbol to be able to resync based on integer STO and CFO estimate, and +1 for fractional sto
                    }
                    ...
                    { 
                        preamble_cfoint_sto_sfo[ii] = in[synchro_read_offset+ (candidate_id -1) * m_samples_per_symbol + m_os_factor * ii] * gr_expj(-2 * M_PI * new_sync_candidate.cfo_frac / m_bins_per_symbol * ii);
                    }
                    ...
                    for (int ii = 0; ii < preamble_tmp.size(); ii++)
                    {
                        ...
                        gr_complex sfo_comp = gr_expj(-2*M_PI*(pow(m_bw,2)*pow(mod(ii, N), 2)/(2*N)*(pow(fs,2)-pow(fs_p,2))/(pow(fs,2)*pow(fs_p,2))+(std::floor((float)ii / N)*(pow(m_bw / fs_p,2)-m_bw / fs_p)+m_bw/2*((fs_p-fs)/(fs*fs_p)))*mod(ii, N)));
                    ...
                    std::cout<<"offset estimate: STO_int: "<<new_sync_candidate.sto_int<<", STO_frac: "<<new_sync_candidate.sto_frac<<", CFO_int: "<<new_sync_candidate.cfo_int<<", CFO_frac: "<<new_sync_candidate.cfo_frac<<", SFO: "<<new_sync_candidate.sfo<<std::endl;
                    ...
                    for (int ii = 0; ii < preamble.size(); ii++)
                    {
                        ...
                        gr_complex sfo_comp = gr_expj(-2*M_PI*(pow(m_bw,2)*pow(mod(ii, N), 2)/(2*N)*(pow(fs,2)-pow(fs_p,2))/(pow(fs,2)*pow(fs_p,2))+(std::floor((float)ii / N)*(pow(m_bw / fs_p,2)-m_bw / fs_p)+m_bw/2*((fs_p-fs)/(fs*fs_p)))*mod(ii, N)));
                        // compensate sto 
                        preamble[ii] = in[synchro_read_offset + (candidate_id -1) * m_samples_per_symbol + (m_bins_per_symbol - new_sync_candidate.sto_int)*m_os_factor + std::lround(m_os_factor * (ii - new_sync_candidate.sto_frac))];
                    ...
                    std::cout<<" Full preamble snr estimate: "<<snr_est_tmp<<std::endl;
                ...
                std::cout << GREEN << "[frame_sync_v2_impl.cc] "<<m_identity<<" "<<work_id <<" frame found with: SNR est symb-by-symb " << symb_by_symb_snr_est << " and all upchirps snr est " << snr_est <<", sto frac :" <<m_sync_candidates[max_idx].sto_frac<< RESET << std::endl;
                std::cout<< GREEN <<"Final offset estimate: STO_int: "<<m_sync_candidates[max_idx].sto_int<<", STO_frac: "<<m_sync_candidates[max_idx].sto_frac<<", CFO_int: "<<m_sync_candidates[max_idx].cfo_int<<", CFO_frac: "<<m_sync_candidates[max_idx].cfo_frac<<", SFO: "<<m_sync_candidates[max_idx].sfo<<RESET<<std::endl;
                
     
                
                if (abs(snr_est - symb_by_symb_snr_est) > m_max_snr_diff)
                {
                    // std::cout << RED << " SNR estimate not consistent" << RESET << std::endl;
                    // m_received_frame_tag = false;
                    m_state = DETECT;
                    goto return_work;
                }
                ...
                items_to_consume = synchro_read_offset + max_idx*m_samples_per_symbol+ std::lround(m_os_factor * (-m_sync_candidates[max_idx].sto_frac)) - m_sync_candidates[max_idx].sto_int * m_os_factor;
                ...
                if (!m_received_frame_tag) // if we didn't received some information via the tags initialise a new one
                {
                    m_frame_tag.value = pmt::make_dict();
                    m_frame_tag.value = pmt::dict_add(m_frame_tag.value, pmt::intern("sf"), pmt::mp((long)m_sf));
                    std::cout <<RED<<"[ERROR][frame_sync] "<<m_identity<<" we didn't received a tag!!\n"<<RESET;
                }
                ...
                m_received_head = true; // TODO received header set to true by hand check effect on normal receiver (not cran joint processing) (was set to false here previously)
                std::cout << "[frame_sync_v2_impl.cc l. 760] "<<m_identity<<" we don't wait for header decoding here, breaks stuff in standard lora receiver!!" << std::endl;
            ...
            case SFO_COMPENSATION:
            {
                ...
                if(m_is_first_sample){
                    ...
                    std::cout<<"[frame_sync_v2] new frame sync time: "<<std::setprecision(6)<<std::fixed<< (m_current_time.get_full_secs()+m_current_time.get_frac_secs()) <<std::defaultfloat<<" s\n";
                ...
                for (int symb_id = 0; symb_id < symbols_to_process; symb_id++)
                {
                    ...
                    if (symbol_cnt < 8 || (symbol_cnt < m_symb_numb && m_received_head) || m_symb_numb==0)
                    {
                        ...
                        {
                            out[ii + m_bins_per_symbol * symb_id] = in[ii * m_os_factor + m_samples_per_symbol * symb_id];
                        }
            ...
            default:
            {
                std::cout<<RED<<"[frame_sync_v2_impl.cc] Should not fall into default state!\n";
