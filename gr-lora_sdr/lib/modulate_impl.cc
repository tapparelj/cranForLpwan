...
namespace gr
{
    namespace lora_sdr
    {
        ...
        modulate::sptr
        modulate::make(uint8_t sf, uint32_t samp_rate, uint32_t bw, std::vector<uint16_t> sync_words, uint32_t frame_zero_padd, uint16_t preamble_len = 8, uint32_t n_frames = 0)
        {
            return gnuradio::get_initial_sptr(new modulate_impl(sf, samp_rate, bw, sync_words, frame_zero_padd, preamble_len, n_frames));
        }
        /*
     * The private constructor
     */
        modulate_impl::modulate_impl(uint8_t sf, uint32_t samp_rate, uint32_t bw, std::vector<uint16_t> sync_words, uint32_t frame_zero_padd, uint16_t preamble_len, uint32_t n_frames)
            : gr::block("modulate",
     ...
     */
                                        ...
                                        gr_vector_void_star &output_items)
        {
            ...
            int output_offset = 0;
            if(m_n_frames>0 && frame_cnt>=m_n_frames)
            {
                return WORK_DONE;
            }
            // read tags
            ...
            if ( samp_cnt == m_frame_len*m_samples_per_symbol + m_inter_frame_padding)
            {
                ...
                if(m_n_frames>0 && frame_cnt>=m_n_frames)
                {
                    std::cout<<"Sent "<<frame_cnt<<" frames, terminating flowgraph.\n";
                    // send custom termination signal to the process
                    kill(getpid(),SIGUSR1);
