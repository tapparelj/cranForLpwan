...
namespace gr
{
     ...
     */
        ...
        void whitening_impl::forecast(int noutput_items, gr_vector_int &ninput_items_required) {
     ...
     */
        ...
        void whitening_impl::msg_handler(pmt::pmt_t message)
        {
            ...
            std::cout<<"Sending: "<<pmt::symbol_to_string(message)<<std::endl;
            // std::copy(payload_str.begin(), payload_str.end(), std::back_inserter(m_payload));
        }

        int whitening_impl::general_work(int noutput_items,
                                         ...
                                         gr_vector_void_star &output_items)
        {
            ...
            if (input_items.size())
            {
                ...
                std::string s;
                for (int i = 0; i < ninput_items[0]; i++) //read payload
                {
                    items_to_consume++;
                    if (in[i] == ',')
                    {
            ...
            if (payload_str.size() && noutput_items >= (m_is_hex?1:2) * payload_str.front().length())
            {
                ...
                items_to_output = 2 * m_payload.size();

                m_payload.clear();
                payload_str.erase(payload_str.begin());
            }
            // std::cout<<"whitening output "<<items_to_output<<" items\n";
