    ...
    namespace lora_sdr
    {
                                         ...
                                         gr_vector_void_star &output_items)
        {
            ...
            if (output_items.size())
            {
                ...
                if (output_crc_check)
                    out_crc = (uint8_t *)output_items[1];
            }
            ...
            if (tags.size())
            {
                ...
                m_has_error = pmt::to_long(pmt::dict_ref(tags[0].value, pmt::string_to_symbol("err"), err));
                current_tag = tags[0];
            ...
            if (m_has_error)
            {
                ...
                return 1; // output one value to propagate error tag (for CRAN)
            }

            else if ((in_buff.size() >= (int)m_payload_len + 2) && m_crc_presence)
            { // wait for all the payload to come

                if (m_payload_len < 2)
                { // undefined CRC
                    std::cout << RED << "CRC not supported for payload smaller than 2 bytes" << RESET << std::endl;
                    m_crc = 0;
                }
                else
                {
                ...
                if (output_crc_check && m_payload_len > 2)
                {
                    out_crc[0] = crc_valid;
                    produce(1, 1);
                }
