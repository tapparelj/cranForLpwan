  ...
  namespace lora_sdr {
    ...
    class modulate_impl : public modulate
    {
     private:
        ...
        uint32_t m_n_frames; ///< number of frames we should transmitt before terminating


     public:
      modulate_impl(uint8_t sf, uint32_t samp_rate, uint32_t bw, std::vector<uint16_t> sync_words, uint32_t frame_zero_padd, uint16_t preamb_len, uint32_t n_frames);
      ~modulate_impl();
