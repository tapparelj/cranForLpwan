...
namespace gr
{
     ...
     */
        ...
        {           
            m_frame_tag.value = pmt::dict_add(m_frame_tag.value, pmt::intern("cr"), pmt::from_long(cr));
            m_frame_tag.value = pmt::dict_add(m_frame_tag.value, pmt::intern("pay_len"), pmt::from_long(pay_len));
            m_frame_tag.value = pmt::dict_add(m_frame_tag.value, pmt::intern("crc"), pmt::from_long(crc));
            m_frame_tag.value = pmt::dict_add(m_frame_tag.value, pmt::intern("ldro_mode"), pmt::from_long(ldro_mode));
            m_frame_tag.value = pmt::dict_add(m_frame_tag.value, pmt::intern("err"), pmt::from_long(err));
            message_port_pub(pmt::intern("frame_info"), m_frame_tag.value);
            m_frame_tag.offset = nitems_written(0);
            // if(!err) //don't propagate downstream that a frame was detected //TODO do it for CRAN worker termination
            ...
            if (is_header)
            {
                ...
                { //explicit header to decode
                    ...
                    if (head_err||m_payload_len==0)
                    {
                        ...
                        head_err = 1;
                        // ----- CRAN  ONLY-------
                        // set to 2 (for terminating worker) output two item with a tag indicating a header error (two required by the dewhitening) 
                        // if payload length is 0 but 1 such that the tag get propagated
                        if(m_payload_len==0){
                            m_payload_len=1;
                        }
                        noutput_items = 2;                
