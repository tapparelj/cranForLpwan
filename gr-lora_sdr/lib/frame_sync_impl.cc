...
namespace gr
{
    namespace lora_sdr
    {
                        ...
                        gr::io_signature::make2(1, 2, sizeof(gr_complex), sizeof(float)))
        {
            ...
            frame_cnt = 0;
            m_symb_numb = 1000;//set large default number of symbol necessary for CRAN, and doesn't matter for not CRAN
        ...
        float frame_sync_impl::determine_snr(const gr_complex *samples, float* out_noise_var, float* out_signal_pow)
        {
            double tot_en = 0;
            float fft_mag_sq[m_number_of_bins];
            std::vector<gr_complex> dechirped(m_number_of_bins);
            ...
            for (uint32_t i = 0u; i < m_number_of_bins; i++)
            {
                fft_mag_sq[i] = cx_out[i].r * cx_out[i].r + cx_out[i].i * cx_out[i].i;
                tot_en += fft_mag_sq[i]/m_number_of_bins;
            }
            free(cfg);

            int max_idx = (std::max_element(fft_mag_sq, fft_mag_sq + m_number_of_bins) - fft_mag_sq);
            float sig_en = fft_mag_sq[max_idx]/m_number_of_bins;

            *out_noise_var += (tot_en - sig_en)/m_number_of_bins; 
            *out_signal_pow += sig_en/m_number_of_bins;//we want the signal power (energy per sample)
                                          ...
                                          gr_vector_void_star &output_items)
        {
            ...
            if(finished()){
                std::cout<<RED<<"work done frame_sync"<<std::endl;
            ...
            if (tags.size())
            {
                //std::cout << RED << "got tag at offset " << tags[0].offset << ", nitemsread: " << nitems_read(0) << ", m_number_of_bins " << m_number_of_bins << RESET << std::endl;
            ...
            //Issue of half buffer when switching SF 
            //if (nitems_to_process < m_number_of_bins*m_os_factor) // empty input buffer if not enough samples to process one additional symbol at the current SF
            // {
            ...
            case DETECT:
            {
                ...
                if (abs(mod(abs(bin_idx_new - bin_idx) + 1, m_number_of_bins) - 1) <= 2 && bin_idx_new != -1) // look for consecutive reference upchirps(with a margin of ±2)
                {
                ...
                bin_idx = bin_idx_new;
                // if (symbol_cnt >=3)
                //  std::cout<<"symb idx: "<<bin_idx<<std::endl;
                if (symbol_cnt == (int)(m_n_up_req))
                {
                    ...
                    // for (size_t i = 0; i < m_n_up_req; i++)
                    // {
                    //     std::cout<<preamb_up_vals[i]<<" ";
                    // }
                    // std::cout<<std::endl;
            ...
            case SYNC:
            {
                ...
                case QUARTER_DOWN:
                {
                    ...
                    for (int i = 0; i < up_symb_to_use; i++)
                    {
                        snr_est += determine_snr(&corr_preamb[i * m_number_of_bins],&m_noise_var,&m_signal_pow);
                    }
                    snr_est /= up_symb_to_use;
                    m_noise_var /= up_symb_to_use;
                    m_signal_pow /= up_symb_to_use;
                    std::cout<<"sig pow and noise var: "<<m_signal_pow<<" "<<m_noise_var<<std::endl;
                    ...
                    if (m_state != DETECT)
                    {
                        ...
                        m_frame_tag.value = pmt::dict_add(m_frame_tag.value, pmt::intern("cfo_int"), pmt::mp((long)m_cfo_int));
                        m_frame_tag.value = pmt::dict_add(m_frame_tag.value, pmt::intern("cfo_frac"), pmt::mp((float)m_cfo_frac));
                        m_frame_tag.value = pmt::dict_add(m_frame_tag.value, pmt::intern("noise_var"), pmt::mp((float)m_noise_var));
                        m_frame_tag.value = pmt::dict_add(m_frame_tag.value, pmt::intern("sig_pow"), pmt::mp((float)m_signal_pow));

                        m_frame_tag.key = pmt::intern("frame_info");
		                m_frame_tag.offset = nitems_written(0);
                        add_item_tag(0, m_frame_tag);
                        m_received_head = false; 
                        std::cout<<"[frame_sync_impl.cc l. 839] we don't wait for header decoding here might break stuff in standard lora receiver!!"<<std::endl;
